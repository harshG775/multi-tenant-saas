...
- **Instance**: [src/lib/auth/auth.ts](../src/lib/auth/auth.ts) — `betterAuth()` with the `drizzleAdapter` (Postgres) and the `tanstackStartCookies()` plugin. This instance is the **site-facing** instance — it authenticates each site domain's own end users (see "Two identity spaces" below), not tenant owners. `basePath: "/api/auth"`, `cookiePrefix: "tenant-user"`.
- **Admin instance**: [src/lib/auth/admin-auth.ts](../src/lib/auth/admin-auth.ts) — the tenant/site-owner instance described below, `basePath: "/api/auth-admin"`, `cookiePrefix: "tenant-admin"`, backed by the `admin`/`adminSession`/`adminAccount`/`adminVerification` tables in [admin.schema.ts](../src/lib/db/schema/admin.schema.ts).
- **Barrel**: [src/lib/auth/index.ts](../src/lib/auth/index.ts) re-exports `auth` (from `auth.ts`) and `adminAuth` (from `admin-auth.ts`) — import both from `#/lib/auth`, never reach into the individual files directly.
...
| | `auth` (exists) | `adminAuth` (exists — routes/UI still to come) |
| --- | --- | --- |
| Who | Each site domain's own end users | Tenant/site owners |
| Schema | `user` / `session` / `account` / `verification`, `user.siteDomainId` FK | `admin` / `adminSession` / `adminAccount` / `adminVerification`, `admin.tenantId` FK |
| Scope | One site domain | One tenant (i.e. every site domain under it) |
| Used at | `tenant-x.com` (the site itself) | `platform.com/p/onboarding`, `platform.com/p/dashboard`, `tenant-x.com/admin` (routes not built yet) |
| `basePath` | `/api/auth` | `/api/auth-admin` |
| `cookiePrefix` | `tenant-user` | `tenant-admin` |
...
3. `trustedOrigins` in `src/lib/auth/auth.ts` (and equivalently in `admin-auth.ts`) reads `x-tenant-id` off the request: present → the origin is trusted (tenantMiddleware already proved it's a real tenant host); absent → `[]` (untrusted).
...
- **`admin` UI/routes** — schema, `adminAuth` instance, and `/api/auth-admin/$` exist; `/p/onboarding`, `/p/dashboard`, and the `tenant-x.com/admin` route guard do not yet. First admin is being created manually (no onboarding flow) to unblock building the dashboard.
- **`PLATFORM_HOST` branch in `tenantMiddleware`** — needed before `/p/*` routes can exist. `adminAuth`'s `trustedOrigins` currently only trusts hosts where `tenantMiddleware` already stamped `x-tenant-id` (same as `auth`); it does not yet special-case the platform's own host.
- **Admin migration generated, not yet applied** — `pnpm db:generate` produced the `admin`/`admin_session`/`admin_account`/`admin_verification` tables (plus the earlier still-pending `tenant`/`site_domain`/`user.site_domain_id` changes) in one migration; `pnpm db:migrate` hasn't been run against the real database yet.
